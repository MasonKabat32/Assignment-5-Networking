...
void GLViewNewModule::updateWorld()
{
   ...
   //Need to listen to incoming messages here
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   if (key.keysym.sym == 49) //1 key
   {
       Vector coords;
       NetMessengerStreamBuffer os;
       if (this->gulfstream != nullptr && this->msg != nullptr && this->os != nullptr && this->is != nullptr)
       {
           this->msg->size = 50;
           this->msg->xPos = coords.x;
           client->sendNetMsgSynchronousTCP(*this->msg);
           this->msg->onMessageArrived();
       }
   }
   else if (key.keysym.sym == 50) { //2 key to create the client for both instances
       client = NetMessengerClient::New("192.168.1.193", "12683");
       std::cout << "Client Created..." << std::endl;
   }
   else if (key.keysym.sym == 52)
   {
       this->moveCam->size = 50;
       client->sendNetMsgSynchronousTCP(*this->moveCam);
   }
   else if (key.keysym.sym == 119) {
       this->cam->moveInLookDirection();
   }
   else if (key.keysym.sym == 97) {
       this->cam->moveLeft();
   }
   else if (key.keysym.sym == 115) {
       this->cam->moveOppositeLookDirection();
   }
   else if (key.keysym.sym == 100) {
       this->cam->moveRight();
   }
   else if (key.keysym.sym == 51) //3 key
   {
       if (!this->spawned)
       {
           this->spawnWO->size = 50;
           client->sendNetMsgSynchronousTCP(*this->spawnWO);
           this->spawnWO->onMessageArrived();
           this->spawned = true;
       }
   }
}
...
void Aftr::GLViewNewModule::loadMap()
{
   ...
   //this->client = NetMessengerClient::New("192.168.1.193", "12683"); //Create the client DO NOT FORGET TO CHANGE ON PC
   this->msg = new Messenger; //unfortunately we need to deallocate this manually at some point
   this->moveCam = new MoveCam;
   this->os = new NetMessengerStreamBuffer;
   this->is = new NetMessengerStreamBuffer;
   this->camIs = new NetMessengerStreamBuffer;
   this->camOs = new NetMessengerStreamBuffer;
   this->spawnWO = new SpawnWO;
   this->spawnIs = new NetMessengerStreamBuffer;
   this->spawnOs = new NetMessengerStreamBuffer;

   //create the lambda to subscribe to the callback here probably
   //callback_on_NetMsgT_arrived<Messenger> callback = [this](auto msg)
   //{    
   //        //clearly not reading in the message correctly
   //        if (msg->fromStream(*this->is))
   //        {
   //            msg->onMessageArrived();
   //            std::cout << "Plane should have moved...ON ITS OWN!!!!!!!\n";
   //            std::cout << "Message contents: " << msg->toString() << "\n";
   //        }
   //};

   //this->subscribe_NetMsg_to_callback<Messenger>(callback); //make sure to listen for any incoming messages
...

   {
      ...
      this->gulfstream->setPosition( Vector( 0, 0, 15 ) );
